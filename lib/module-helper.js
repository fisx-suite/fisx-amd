...
exports.getAst = function (code, options) {
    return require('esprima').parse(
        code,
        assign({attachComment: false}, options || {})
    );
};
 ...
 * @return {{id: string, query: string, path: string}}
 */
function getResourceInfo(resId, ownerId, moduleConfig, mapConfigs) {
    ...
    var file = exports.getModuleFile(id, moduleConfig);
    var relativePath = file && (file = path.relative(moduleConfig.root, file));
    return {
        rawId: resId,
        id: id.replace(constant.MODULE_EXTNAME_REGEXP, ''),
        query: query,
        path: relativePath,
        fullPath: file
    };
 ...
 * @param {string=} extName 要附加的文件后缀名
 * @return {string}
 */
function normalizeModuleFilePath(filePath, moduleConfig, extName) {
    // 只有文件路径不包含文件后缀名时候才加上指定的后缀名
    var currExt = extReg.exec(filePath);
    currExt = currExt && currExt[1];
    var optionalExtNames = moduleConfig.extNames || [constant.MODULE_EXTNAME];
    if (!currExt || optionalExtNames.indexOf(currExt) === -1) {
        filePath += (extName || constant.MODULE_EXTNAME);
    }
    return filePath;
}
...
exports.initModuleConfig = function (moduleConfig, root) {
    ...
    for (var i = 0, len = packages.length; i < len; i++) {
        ...
        if (pkgMain) {
            pkg.main = pkgMain.replace(constant.MODULE_EXTNAME_REGEXP, '');
        }
    }
...
exports.getModuleId = function (moduleFile, moduleConfig, returnDefault) {
    ...
    var moduleFilePath = path.resolve(
        root, moduleFile.replace(constant.MODULE_EXTNAME_REGEXP, '')
    );
...
exports.getModuleFile = function (moduleId, moduleConfig, extname) {
    ...
    for (var i = 0, len = packages.length; i < len; i++) {
        ...
        if (moduleId.split('/')[0] === pkgName) {
            ...
            return normalizeModuleFilePath(
                path.resolve(root, baseUrl, pkgDir, moduleId),
                moduleConfig, extname
            );
        }
    }
    ...
    for (i = 0, len = pathMaps.length; i < len; i++) {
        ...
        if (item.reg.test(moduleId)) {
            ...
            return normalizeModuleFilePath(
                path.resolve(root, baseUrl, modulePath, moduleId),
                moduleConfig, extname
            );
        }
    }
    ...
    return normalizeModuleFilePath(
        path.resolve(root, baseUrl, moduleId),
        moduleConfig, extname
    );
};
