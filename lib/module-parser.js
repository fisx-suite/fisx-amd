...
/**
 * 分析 define 调用，获取模块信息
 * TODO require('er/tpl!xxx') 需要提取插件的依赖: er/tpl?
 *
 * @inner
 ...
 * 解析 amd 模块信息
 *
 * @param {Object|string} ast 抽象语法树 或者 源码
 * @param {boolean=} parseNotDefineCode 是否解析非 define 代码，可选，默认 false
 * @return {?Array.<Object>}
 */
module.exports = exports = function (ast, parseNotDefineCode) {
    if (typeof ast === 'string') {
        var helper = require('./module-helper');
        ast = helper.getAst(ast);
    }
    ...
    var hasGlobalRequire = syncDeps.length || asynDeps.length;
    if (hasGlobalRequire && modules.length) {
        console.error(
            'global require existed with define moudle codes is not allowd, '
            + 'it will ignore the global require information: %j, %j',
            syncDeps, asynDeps
        );
    }

    // XXX: 这里假设传入代码要么都是 define 模块化代码，要么只是 require 的入口代码
    if (modules.length) {
        return modules;
    }
